...
// Returns one stored Competitive Benchmarker card by its unguessable token.
...
exports.handler = async (event) => {
  ...
  try {
    const r = await fetch(
      `${SUPABASE_URL}/rest/v1/benchmark_reports?select=report&token=eq.${encodeURIComponent(token)}&limit=1`,
